<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
		...
		<dl class="dl dl4">
            <dt><h2><a class="a12" href="${ctx}/${key}_news/p${page.pageNo}.htm" title="News">News</a></h2></dt>
            <dd>
            	<c:if test="${fn:length(page.result) gt 0}">
            	<ul class="lts public_ul">
            		<c:forEach items="${page.result}" var="news">
            		<li>
						<h2 title="${news.title}"><a href="${ctx}/${key}_news/info/${st:replace(news.title, regs)}-${news.id}.htm" title="${news.title}">${news.title}</a></h2>
